...
- Gate 3: verified product, approve release`,
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			for (const q of params.questions) {
				...
				if (q.multi) {
					// Multi-select via repeated single-select with checkbox indicators
					const selected = new Set<number>();
					let done = false;

					while (!done) {
						const opts = [
							...labels.map((l, i) => (selected.has(i) ? `☑ ${l}` : `☐ ${l}`)),
							...(selected.size > 0 ? ["✓ Confirmar seleção"] : []),
						...
						// Find which label was selected by matching the index in the options list
						const choiceIndex = opts.indexOf(choice);
					...
					if (!results.find((r) => r.id === q.id)) {
						const selectedLabels = [...selected].map((i) => labels[i]);
						results.push({ id: q.id, selected: selectedLabels, custom: undefined });
					}
				} else {
					// Single select
